...
~/.glance/openclaw.toml — written by install.sh after asking the user.
"""
...
PR_HOME = Path(os.environ.get("GLANCE_HOME", Path.home() / ".glance"))
...
def _write_jobs(data: dict[str, Any]) -> None:
    ...
    backup = JOBS_PATH.with_suffix(f".json.bak.glance.{int(time.time())}")
